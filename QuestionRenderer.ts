...
export function displayQuestion(q: QuestionData[], curr: number): HTMLElement {

    var div = document.getElementById("quizDisplay") as HTMLElement;
    div.innerText = "";
    ...
    for (let i = 0; i < q[curr].answers.length; i++) {
        ...
        answerDisplay.addEventListener("click", event => {
            ...
            const allAnswers = document.querySelectorAll<HTMLDivElement>('div.answer');
            allAnswers.forEach(btn => btn.classList.remove('gbg'));
            //add bbg class to new selected
            (event.currentTarget as HTMLElement).classList.add("gbg");
            //add to answers
            answers[curr] = i;
        })
